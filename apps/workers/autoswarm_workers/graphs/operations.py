...
def check_inventory(state: OperationsState) -> OperationsState:
    ...
    if sku:
        try:
            from autoswarm_tools.builtins.operations import InventoryCheckTool
...
def process_pedimento(state: OperationsState) -> OperationsState:
    ...
    if numero:
        try:
            from autoswarm_tools.builtins.operations import PedimentoLookupTool
...
def track_shipments(state: OperationsState) -> OperationsState:
    ...
    for entry in tracking_entries:
        ...
        if not carrier or not number:
            continue
        try:
            from autoswarm_tools.builtins.operations import CarrierTrackingTool
