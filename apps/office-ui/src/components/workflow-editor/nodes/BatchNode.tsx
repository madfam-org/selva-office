...
export function BatchNode({ data, selected }: NodeProps) {
  const nodeData = data as unknown as NodeDefinition;
  return (
    <div className={`retro-panel pixel-border min-w-[160px] bg-slate-800/95 shadow-pixelact-raised ${selected ? 'ring-2 ring-teal-400' : ''}`}>
      <div className="flex items-center gap-2 px-2 py-1.5 border-b border-pixelact-border bg-teal-500/20">
        <span>📦</span>
      ...
      <div className="px-2 py-1.5 text-retro-xs text-slate-400 space-y-0.5">
        <div>split: {nodeData.batch_split_key ?? '—'}</div>
        <div>strategy: {nodeData.batch_aggregate_strategy ?? 'collect'}</div>
        <div>max ∥: {nodeData.max_parallel ?? 5}</div>
      </div>
